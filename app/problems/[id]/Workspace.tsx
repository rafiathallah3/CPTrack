...
export default function Workspace({ problem, hasUserSolved }: { problem: any; hasUserSolved?: boolean }) {
  ...
  // Custom components for ReactMarkdown to fulfill user's aesthetic requirements
  const markdownComponents: any = {
    // Style variable names (rendered as italics/em) with Times New Roman
    em: ({ node, children, ...props }: any) => {
      const text = React.Children.toArray(children).join("");
      if (text.includes("_")) {
        const parts = text.split("_");
        return (
          <i style={{ fontFamily: '"Times New Roman", Times, serif', fontStyle: 'italic', fontSize: '1.15em' }}>
            {parts[0]}
            <sub style={{ fontSize: '0.65em', verticalAlign: 'sub', marginLeft: '1px', fontStyle: 'normal' }}>{parts[1]}</sub>
          </i>
        );
      }
      return (
    ...
    // Style headers (rendered as strong) to have a line break and spacing
    strong: ({ node, children, ...props }: any) => {
      const text = React.Children.toArray(children).join("");
      const isHeader = [
        'Input Format', 'Output Format', 'Constraints', 'Batasan',
        'Explanation', 'Sample Input', 'Sample Output', 'Notes'
      ].some(h => text.toLowerCase().includes(h.toLowerCase()));

      if (isHeader) {
        return (
          <span className="block mt-8 mb-4">
            <strong className="text-cyan-400 uppercase tracking-widest border-b border-cyan-900/50 pb-1" {...props}>
              {children}
            </strong>
          </span>
        );
    ...
    // Also style inline code just in case
    code: ({ node, inline, className, children, ...props }: any) => {
      if (inline) {
        return (
          <code
            className="font-serif italic text-white bg-transparent p-0"
            style={{ fontFamily: '"Times New Roman", Times, serif', fontSize: '1.1em' }}
            {...props}
          >
            {children}
          </code>
  ...
  const formatText = (text: string) => {
    if (!text) return "";
    return text
      .replace(/\b([B-HJ-NP-Z](?:_[a-zA-Z0-9]+)?|A_[a-zA-Z0-9]+|I_[a-zA-Z0-9]+|n|m|k|u|v|x|y|z|i|j)\b/g, '*$1*')
      .replace(/([A-Z])(?=\s*[≤<>=])/g, '*$1*');
  };
            ...
            <div className="space-y-8">
              <header className="space-y-4 border-b border-gray-800 pb-8">
                <div className="flex items-center gap-4">
                  <span className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest border ${problem.difficulty === 'Easy' ? 'bg-green-500/10 text-green-400 border-green-500/30' :
                      ...
                      <div className="flex gap-2 p-2 border-b border-gray-800 bg-[#050505] overflow-x-auto flex-shrink-0 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
                        ...
                        {executionResult.results.map((res, i) => (
                          <button
                            ...
                            className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest border transition-colors flex items-center gap-2 ${activeTab === i
                        ...
                        {executionResult.hiddenResults && executionResult.hiddenResults.map((res, i) => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest border transition-colors flex items-center gap-2 ${activeTab === tabIdx
                                ? (res.passed ? 'bg-green-500/20 text-green-400 border-green-500/50' : 'bg-red-500/20 text-red-500 border-red-500/50')
                                : 'bg-transparent text-gray-400 border-gray-800 hover:border-gray-600'
                                }`}
                            >
            ...
            {workflowState !== 'idle' && (
              <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-gray-600">
                <div className="flex items-center gap-3">
                  <span className={`w-1.5 h-1.5 rounded-full ${workflowState === 'sample_passed' ? 'bg-yellow-500' :
